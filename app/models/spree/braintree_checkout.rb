  ...
  class BraintreeCheckout < ActiveRecord::Base
    ...
    def self.update_states
      ...
      not_in_state(FINAL_STATES).find_each do |checkout|
        checkout.state = get_braintree_checkout_status(checkout)
      ...
      complete_failed_orders_with_settled_checkout(recent_with_paypal, braintree_transaction)
      result
    end
    ...
    def complete_failed_orders_with_settled_checkout(recent_paypal_checkouts)
      recent_paypal_checkouts.each do |checkout|
        next unless failed_order_and_settled_checkout?(checkout, braintree_transaction)
        ...
        order_payment.state = 'completed' if [checkout.order.total,
                                              ...
                                              get_braintree_checkout_amount(checkout)].uniq.length == 1
        order_payment.save!
    ...
    def get_braintree_checkout(checkout)
      braintree::Transaction.find(checkout.transaction_id)
    end

    def get_braintree_checkout_status(checkout)
      get_braintree_checkout(checkout).status
    end

    def get_braintree_checkout_amount(checkout)
      get_braintree_checkout(checkout).amount
    end
